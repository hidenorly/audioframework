...
public:
  ...
  template<typename... Ts> static std::weak_ptr<Derived> getInstance(Ts&&... _args)  {
    if( !mInstance ){
      mInstance = std::make_shared<Derived>(std::forward<Derived>(_args)...);
      if( mInstance ){
        mInstance->_onInstantiate();
      }
    }
    return mInstance;
  }
  ...
  virtual void _onInstantiate(void) { onInstantiate(); };
  virtual void _onFinalize(void) { onFinalize(); };
};
